...
public class Schematic
{
    ...
    public static List<GladiatorBlock> spawnGladiator(final Player gladiator, final Player target, final Location loc, final Schematic schematic) {
        ...
        for (int x = 0; x < width; ++x) {
            for (int y = 0; y < height; ++y) {
                for (int z = 0; z < length; ++z) {
                    ...
                    final Block b = new Location(loc.getWorld(), x + loc.getX(), y + loc.getY(), z + loc.getZ()).getBlock();
                    Schematic.asyncWorldEdit.setAsyncBlock(loc.getWorld(), b.getLocation().getBlockX(), b.getLocation().getBlockY(), b.getLocation().getBlockZ(), (int)blocks[index], blockData[index]);
                }
            }
        }
        gladiator.teleport(new Location(loc.getWorld(), (double)(loc.getBlockX() + 4), (double)(loc.getBlockY() + 2), (double)(loc.getBlockZ() + 4)));
        return list;
    }
        ...
        for (int x = 0; x < width; ++x) {
            for (int y = 0; y < height; ++y) {
                for (int z = 0; z < length; ++z) {
                    ...
                    Schematic.asyncWorldEdit.setAsyncBlock(world, block.getX(), block.getY(), block.getZ(), (int)blocks[index], blockData[index]);
                }
            }
        }
    }
