...
public class GrapplerAbility extends Ability
{
    ...
    public void onPlayerInteract(final PlayerInteractEvent event) {
        ...
        if (event.getAction().name().contains("LEFT")) {
            ...
            final double t;
            final double d = t = loc.distance(p.getLocation());
            final double v_x = (1.0 + 0.04000000000000001 * t) * ((this.isNear(loc, pLoc) ? 0.0 : (loc.getX() - pLoc.getX())) / t);
            final double v_y = (0.9 + 0.03 * t) * ((this.isNear(loc, pLoc) ? 0.1 : (loc.getY() - pLoc.getY())) / t);
            final double v_z = (1.0 + 0.04000000000000001 * t) * ((this.isNear(loc, pLoc) ? 0.0 : (loc.getZ() - pLoc.getZ())) / t);
    ...
    public void onPlayerLeash(final PlayerLeashEntityEvent event) {
        ...
        if (this.hooks.containsKey(p.getUniqueId()) && this.hooks.get(p.getUniqueId()).isHooked()) {
            ...
            final double t;
            final double d = t = loc.distance(playerLoc);
            final double v_x = (1.0 + 0.04000000000000001 * t) * ((this.isNear(loc, playerLoc) ? 0.0 : (loc.getX() - playerLoc.getX())) / t);
            final double v_y = (0.9 + 0.03 * t) * ((this.isNear(loc, playerLoc) ? 0.1 : (loc.getY() - playerLoc.getY())) / t);
            final double v_z = (1.0 + 0.04000000000000001 * t) * ((this.isNear(loc, playerLoc) ? 0.0 : (loc.getZ() - playerLoc.getZ())) / t);
            final Vector v = p.getVelocity();
            v.setX(v_x);
            v.setY(v_y);
            v.setZ(v_z);
