...
public class SpiderAbility extends Ability
{
    ...
    public void onInteract(final PlayerInteractEvent event) {
        ...
        if (this.hasKit(p)) {
            ...
            if (itemInHand.getType() == this.getIcon() && itemInHand.hasItemMeta() && itemInHand.getItemMeta().getDisplayName().equals("§e§l" + this.getName())) {
                event.setCancelled(true);
                if (!this.inCooldown(p)) {
                    this.addCooldown(p, 25);
                    p.throwSnowball().setMetadata("Spiderball", (MetadataValue)new FixedMetadataValue((Plugin)yPvP.getPlugin(), (Object)"Spiderball"));
                }
                else {
                    this.sendCooldown(p);
                }
