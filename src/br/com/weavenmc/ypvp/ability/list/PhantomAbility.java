...
public class PhantomAbility extends Ability
{
                            ...
                            public void run() {
                                if (current == 5) {
                                    ...
                                    if (PhantomAbility.this.previous.containsKey(p.getUniqueId())) {
                                        p.getInventory().setArmorContents((ItemStack[])PhantomAbility.this.previous.get(p.getUniqueId()));
                                        p.updateInventory();
                                        PhantomAbility.this.previous.remove(p.getUniqueId());
                                    }
                                }
                                else if (PhantomAbility.this.flying.contains(p.getUniqueId())) {
                                    p.sendMessage("§5§lPHANTOM§f Voc\u00ea n\u00e3o voar\u00e1 mais em §9§l" + PhantomAbility.this.convert(current) + " SEGUNDOS...");
                                }
                            }
    ...
    public void onPhantonBug(final InventoryClickEvent event) {
        if (event.getWhoClicked() instanceof Player) {
            ...
            if ((this.flying.contains(p.getUniqueId()) || this.previous.containsKey(p.getUniqueId())) && event.getSlotType() == InventoryType.SlotType.ARMOR) {
                event.setCancelled(true);
            }
        }
    }
