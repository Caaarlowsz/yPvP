    ...
    public void onSupernovaListener(final PlayerInteractEvent event) {
        ...
        if (this.hasKit(p) && this.isItem(p.getItemInHand()) && event.getAction().toString().contains("RIGHT")) {
            ...
            for (final ArrowDirection d : this.directions) {
                synchronized (this) {
                    final Arrow arrow = (Arrow)loc.getWorld().spawn(loc.clone().add(0.0, 1.0, 0.0), (Class)Arrow.class);
                    arrow.setMetadata("Supernova", (MetadataValue)new FixedMetadataValue((Plugin)yPvP.getPlugin(), (Object)p.getUniqueId()));
