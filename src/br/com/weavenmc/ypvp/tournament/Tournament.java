    ...
    public void checkWinner() {
        if (this.players.size() <= 1) {
            ...
            if (this.players.size() == 1) {
                ...
                if (winner != null) {
                    Bukkit.broadcastMessage("§3§lTOURNAMENT§f O jogador §a§l" + winner.getName() + "§f foi o §2§lVENCEDOR§f!");
                    final BukkitPlayer bP = (BukkitPlayer)WeavenMC.getAccountCommon().getWeavenPlayer(winner.getUniqueId());
                    bP.addMoney(7000);
                    bP.addXp(25);
                    ...
                    Bukkit.dispatchCommand((CommandSender)Bukkit.getConsoleSender(), "group " + winner.getName() + " add torneio " + tournamentTime);
                }
                else {
                    this.stop();
                }
