                ...
                if (gamer.getAbility().getName().equals("Nenhum")) {
                    ...
                    if (ability != null) {
                        if (bP.hasGroupPermission(Group.COPA) || bP.hasGroupPermission(ability.getGroupToUse()) || this.hasKit(bP, ability)) {
                            ...
                            p.sendMessage("§b§lKITS§f Voc\u00ea n\u00e3o possui o kit §3§l" + ability.getName().toUpperCase() + "§f!");
                        }
                    }
                    else {
    ...
    public List<String> kitcompleter(final BukkitCommandSender sender, final String label, final String[] args) {
        ...
        if (args.length == 1) {
            for (final Ability ability : yPvP.getPlugin().getAbilityManager().getAbilities()) {
                if (bP.hasGroupPermission(ability.getGroupToUse()) || bP.getData(DataType.PLAYER_PERMISSIONS).asList().contains("pvpkit." + ability.getName().toLowerCase())) {
