...
public class PlayerListener implements Listener
{
    public PlayerListener() {
        ...
        this.newShapelessRecipe(soup, Arrays.asList(new MaterialData(Material.CACTUS), new MaterialData(Material.BOWL)));
        this.newShapelessRecipe(soup, Arrays.asList(new MaterialData(Material.NETHER_STALK), new MaterialData(Material.BOWL)));
        this.newShapelessRecipe(soup, Arrays.asList(new MaterialData(Material.INK_SACK, (byte)3), new MaterialData(Material.BOWL)));
        this.newShapelessRecipe(soup, Arrays.asList(new MaterialData(Material.SUGAR), new MaterialData(Material.BOWL)));
        this.newShapelessRecipe(soup, Arrays.asList(new MaterialData(Material.PUMPKIN_SEEDS), new MaterialData(Material.PUMPKIN_SEEDS), new MaterialData(Material.BOWL)));
        this.newShapelessRecipe(soup, Arrays.asList(new MaterialData(Material.CARROT_ITEM), new MaterialData(Material.POTATO_ITEM), new MaterialData(Material.BOWL)));
    }
    ...
    public void onLoginListener(final PlayerLoginEvent event) {
        ...
        if (bP != null) {
            ...
            WeavenMC.getAsynchronousExecutor().runAsync(() -> bP.load(new DataCategory[] { DataCategory.KITPVP, DataCategory.LAVA_CHALLENGE }));
        }
        else {
            event.disallow(PlayerLoginEvent.Result.KICK_OTHER, "§4§lERRO§f Ocorreu um erro ao tentar carregar sua conta.");
        }
    }
    ...
    public void combat(final EntityDamageByEntityEvent event) {
        if (event.getEntity() instanceof Player && event.getDamager() instanceof Player && !event.isCancelled()) {
            ...
            if (gamer.getWarp().getName().equalsIgnoreCase("spawn") || gamer.getWarp().getName().equalsIgnoreCase("fps")) {
    ...
    public void onCreatureSpawn(final CreatureSpawnEvent event) {
        if (event.getSpawnReason() != CreatureSpawnEvent.SpawnReason.DISPENSE_EGG && event.getSpawnReason() != CreatureSpawnEvent.SpawnReason.CUSTOM && event.getSpawnReason() != CreatureSpawnEvent.SpawnReason.SPAWNER_EGG) {
            event.setCancelled(true);
        }
    }
    ...
    public void onPickUp(final PlayerPickupItemEvent event) {
        ...
        else if (!material.toString().contains("MUSHROOM") && material != Material.BOWL && material != Material.ENDER_PEARL && material != Material.EXP_BOTTLE && material != Material.GOLDEN_APPLE && material != Material.CACTUS && material != Material.COCOA && material != Material.GLASS_BOTTLE) {
            event.setCancelled(true);
        }
        material = null;
        gamer = null;
    ...
    public void onStopDeath(final EntityDamageEvent event) {
        if (event.getEntity() instanceof Player) {
            ...
            if (p.getHealth() - event.getFinalDamage() <= 0.0) {
                ...
                Bukkit.getPluginManager().callEvent((Event)new PlayerDeathEvent(p, (List)items, 0, 0, (String)null));
            }
        }
    }
    ...
    public void onPlayerDeathListener(final PlayerDeathEvent event) {
        ...
        if (p.getKiller() != null && p.getKiller() instanceof Player) {
            ...
            killer.sendMessage("§9§lXP§f Voc\u00ea recebeu §9§l" + xp + " XPs" + (bP.isDoubleXPActived() ? " §7(doublexp)" : ""));
    ...
    public void onRepair(final PlayerInteractEvent event) {
        ...
        if (itemStack != null) {
            ...
            if (action.name().contains("LEFT")) {
                if (itemStack.getType() == Material.DIAMOND_SWORD || itemStack.getType() == Material.STONE_SWORD || itemStack.getType() == Material.WOOD_SWORD || itemStack.getType() == Material.STONE_SWORD || itemStack.getType() == Material.IRON_SWORD || itemStack.getType() == Material.GOLD_SWORD || itemStack.getType() == Material.DIAMOND_AXE || itemStack.getType() == Material.GOLD_AXE || itemStack.getType() == Material.STONE_AXE || itemStack.getType() == Material.WOOD_AXE || itemStack.getType() == Material.IRON_AXE || itemStack.getType() == Material.FISHING_ROD) {
    ...
    public void onDropItem(final PlayerDropItemEvent event) {
        ...
        if (gamer.getWarp().getName().equals("1v1")) {
            ...
            if (!material.toString().contains("MUSHROOM") && material != Material.CACTUS && material != Material.BOWL && material != Material.ENDER_PEARL && material != Material.EXP_BOTTLE && material != Material.GOLDEN_APPLE && material != Material.GLASS_BOTTLE && !material.toString().contains("_BOOTS") && !material.toString().contains("_LEGGINGS") && !material.toString().contains("_CHESTPLATE") && !material.toString().contains("_HELMET")) {
                event.setCancelled(true);
            }
            material = null;
        }
    ...
    public void onSoup(final PlayerInteractEvent event) {
        ...
        if ((event.getAction() == Action.RIGHT_CLICK_BLOCK || event.getAction() == Action.RIGHT_CLICK_AIR) && p.getHealth() < p.getMaxHealth()) {
