...
public class VoidChallengeMinigame extends Minigame
{
    ...
    public String timerFormat(final int time) {
        if (time >= 3600) {
            ...
            return String.valueOf((hours < 10) ? "0" : "") + hours + ":" + ((minutes < 10) ? "0" : "") + minutes + ":" + ((seconds < 10) ? "0" : "") + seconds;
        }
    ...
    public String getTimeFormat(final int time) {
        if (time >= 3600) {
            ...
            final int minutes = time % 3600 / 60;
            final int seconds = time % 3600 % 60;
            return String.valueOf((hours == 1) ? new StringBuilder(String.valueOf(hours)).append(" hora").toString() : new StringBuilder(String.valueOf(hours)).append(" horas").toString()) + " " + ((minutes == 1) ? (String.valueOf(minutes) + " minuto") : (String.valueOf(minutes) + " minutos")) + ((seconds <= 0) ? "" : (" e " + ((seconds == 1) ? (String.valueOf(seconds) + " segundo") : (String.valueOf(seconds) + " segundos"))));
        }
        if (time < 60) {
        ...
        final int minutes2 = time / 60;
        final int seconds2 = time % 60;
        return String.valueOf((minutes2 == 1) ? new StringBuilder(String.valueOf(minutes2)).append(" minuto").toString() : new StringBuilder(String.valueOf(minutes2)).append(" minutos").toString()) + ((seconds2 <= 0) ? "" : (" e " + ((seconds2 == 1) ? (String.valueOf(seconds2) + " segundo") : (String.valueOf(seconds2) + " segundos"))));
    }
    ...
    public void onDeath(final PlayerDeathEvent event) {
        ...
        if (this.voidTimers.containsKey(p.getUniqueId())) {
            ...
            p.sendMessage("§5§lVOID CHALLENGE§f Voc\u00ea sobreviveu por §9§l" + this.getTimeFormat(survivalTime).toUpperCase() + "§f!");
            if (survivalTime >= 1) {
