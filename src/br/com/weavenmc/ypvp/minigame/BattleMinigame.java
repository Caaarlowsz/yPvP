...
public class BattleMinigame extends Minigame
{
    ...
    public void onSpectatorBattleEnd(final SpectatorBattleEndEvent event) {
        for (final Gamer gamer : yPvP.getPlugin().getGamerManager().getGamers()) {
            ...
            if (!gamer.getSpectator().equals(event.getPlayer1().getUniqueId()) && !gamer.getSpectator().equals(event.getPlayer2().getUniqueId())) {
                continue;
            }
    ...
    public void onPlayerHideListener(final PlayerHideEvent event) {
        final Player hide = event.getToHide();
        if (this.battle.containsKey(hide.getUniqueId()) && this.battle.get(hide.getUniqueId()) == event.getPlayer().getUniqueId()) {
            event.setCancelled(true);
        }
    }
    ...
    public void onPlayerShowListener(final PlayerShowEvent event) {
        final Player show = event.getToShow();
        if (this.battle.containsKey(show.getUniqueId()) && this.battle.get(show.getUniqueId()) != event.getPlayer().getUniqueId()) {
            event.setCancelled(true);
        }
    }
        ...
        if (itemInHand.getType() == Material.INK_SACK && itemInHand.hasItemMeta()) {
            if (itemInHand.getItemMeta().getDisplayName().equals("§e§l1v1 R\u00e1pido")) {
                if (this.nextBattle == null) {
                    ...
                    p.setItemInHand(new ItemBuilder().type(Material.INK_SACK).durability(10).name("§e§lProcurando partidas").build());
                    p.updateInventory();
                    ...
                    if (finded != null) {
                        if (finded.getUniqueId() != p.getUniqueId()) {
                            ...
                            finded.sendMessage("§9O 1v1 R\u00e1pido encontrou algu\u00e9m para voc\u00ea lutar! O player escolhido foi §e" + p.getName());
                            p.sendMessage("§9O 1v1 R\u00e1pido encontrou algu\u00e9m para voc\u00ea lutar! O player escolhido foi §e" + finded.getName());
                        }
                        else {
                            this.nextBattle = p.getUniqueId();
                            p.setItemInHand(new ItemBuilder().type(Material.INK_SACK).durability(10).name("§e§lProcurando partidas").build());
                ...
                p.setItemInHand(new ItemBuilder().type(Material.INK_SACK).durability(8).name("§e§l1v1 R\u00e1pido").build());
                p.updateInventory();
                ...
                if (!yPvP.getPlugin().getCooldownManager().hasCooldown(p)) {
                    ...
                    if (g.getWarp() == this && !this.battle.containsKey(challenged.getUniqueId())) {
                        if (itemInHand.getType() == Material.BLAZE_ROD) {
                            event.setCancelled(true);
                            if (this.normalChallenge.containsKey(p.getUniqueId()) && this.normalChallenge.get(p.getUniqueId()).contains(challenged.getUniqueId())) {
                                ...
                                p.sendMessage("§7Voc\u00ea enviou um desafio de 1v1 normal para §b" + challenged.getName());
                                challenged.sendMessage("§eVoc\u00ea recebeu desafio de 1v1 normal de §7" + p.getName());
                                final List<UUID> challengers = this.normalChallenge.containsKey(challenged.getUniqueId()) ? this.normalChallenge.get(challenged.getUniqueId()) : new ArrayList<UUID>();
                                ...
                                new BukkitRunnable() {
                                    public void run() {
                                        if (BattleMinigame.this.normalChallenge.containsKey(challenged.getUniqueId())) {
                                            BattleMinigame.this.normalChallenge.get(challenged.getUniqueId()).remove(p.getUniqueId());
                                        }
                                    }
                            ...
                            if (this.customChallenge.containsKey(p.getUniqueId()) && this.customChallenge.get(p.getUniqueId()).equals(challenged.getUniqueId())) {
    ...
    public void openCustomInventoryFor(final Player p, final Player challenged) {
        ...
        if (this.swordType.containsKey(p.getUniqueId())) {
            ...
            if (sword == Material.WOOD_SWORD) {
                builder.name("§6Espada de Madeira").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de sua espada!", "" });
            }
            else if (sword == Material.STONE_SWORD) {
                builder.name("§6Espada de Pedra").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de sua espada!", "" });
            }
            else if (sword == Material.IRON_SWORD) {
                builder.name("§6Espada de Ferro").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de sua espada!", "" });
            }
            else if (sword == Material.DIAMOND_SWORD) {
                builder.name("§6Espada de Diamante").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de sua espada!", "" });
            }
        ...
        if (this.armourType.containsKey(p.getUniqueId())) {
            ...
            if (armour == Material.LEATHER_CHESTPLATE) {
                builder.name("§eArmadura de Couro").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de sua armadura!", "" });
            }
            else if (armour == Material.IRON_CHESTPLATE) {
                builder.name("§eArmadura de Ferro").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de sua armadura!", "" });
            }
            else if (armour == Material.DIAMOND_CHESTPLATE) {
                builder.name("§eArmadura de Diamente").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de sua armadura!", "" });
            }
            else if (armour == Material.GOLD_HELMET) {
                builder.name("§eSem armadura").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de sua armadura!", "" });
            }
        ...
        if (this.recraftType.containsKey(p.getUniqueId())) {
            ...
            if (recraft == Material.RED_MUSHROOM) {
                builder.name("§bRecrafts de Cogumelo").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de seu recraft!", "" });
            }
            else if (recraft == Material.COCOA) {
                builder.name("§bRecrafts de Cocoabean").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de seu recraft!", "" });
            }
            menu.setItem(22, builder.build());
        }
        if (this.recraftOption.containsKey(p.getUniqueId())) {
            if (this.recraftOption.get(p.getUniqueId())) {
                builder = new ItemBuilder().type(Material.BROWN_MUSHROOM).name("§aCom Recraft").lore(new String[] { "§3Clique aqui para", "§3desativar o recraft!", "" });
            }
            else {
                builder = new ItemBuilder().type(Material.MAGMA_CREAM).name("§cSem Recraft").lore(new String[] { "§3Clique aqui para", "§3ativar o recraft!", "" });
            }
            menu.setItem(23, builder.build());
        }
        if (this.sharpOption.containsKey(p.getUniqueId())) {
            if (this.sharpOption.get(p.getUniqueId())) {
                builder = new ItemBuilder().type(Material.ENCHANTED_BOOK).name("§3Com Sharpness").lore(new String[] { "§3Clique aqui para", "§3tirar a afia\u00e7ao da espada!", "" });
            }
            else {
                builder = new ItemBuilder().type(Material.BOOK).name("§3Sem Sharpness").lore(new String[] { "§3Clique aqui para", "§3colocar afia\u00e7ao na espada!", "" });
            }
            menu.setItem(24, builder.build());
        }
        if (this.fullSoupOption.containsKey(p.getUniqueId())) {
            if (this.fullSoupOption.get(p.getUniqueId())) {
                builder = new ItemBuilder().type(Material.MUSHROOM_SOUP).name("§2Full Sopa").lore(new String[] { "§3Clique aqui para", "§3usar 1 hotbar apenas", "" });
            }
            else {
                builder = new ItemBuilder().type(Material.BOWL).name("§21 Hotbar").lore(new String[] { "§3Clique aqui para", "§3usar full sopa", "" });
            }
    ...
    public void onDamageHit(final EntityDamageByEntityEvent event) {
        if (event.getEntity() instanceof Player && event.getDamager() instanceof Player) {
            ...
            if (entity.getWarp() == this || damager.getWarp() == this) {
                ...
                else if (this.battle.containsKey(e.getUniqueId()) && this.battle.containsKey(d.getUniqueId()) && this.battle.get(d.getUniqueId()) != e.getUniqueId() && this.battle.get(e.getUniqueId()) != d.getUniqueId()) {
                    event.setCancelled(true);
                }
            }
        }
    ...
    public void onBattleLogout(final PlayerQuitEvent event) {
        ...
        if (this.battle.containsKey(logout.getUniqueId())) {
            final BukkitPlayer bPLoser = (BukkitPlayer)WeavenMC.getAccountCommon().getWeavenPlayer(logout.getUniqueId());
            ...
            final BukkitPlayer bPWinner = (BukkitPlayer)WeavenMC.getAccountCommon().getWeavenPlayer(winner.getUniqueId());
            ...
            winner.sendMessage("§9§lXP§f Voc\u00ea recebeu §9§l" + xp + " XPs" + (bPWinner.isDoubleXPActived() ? " §7(doublexp)" : ""));
    ...
    public void checkLostKs(final Player p, final Player k, final int ks) {
        ...
        Bukkit.broadcastMessage("§4§lKILLSTREAK §1§l" + p.getName() + "§f perdeu seu §6§lKILLSTREAK DE " + ks + " PARA §c§l" + k.getName());
    }
    ...
    public void onLose(final PlayerDeathEvent event) {
        ...
        if (this.battle.containsKey(loser.getUniqueId())) {
            ...
            winner.sendMessage("§cVoc\u00ea venceu o 1v1 contra " + loser.getName() + " com " + restingLife + " cora\u00e7oes e " + restingSoups + " sopas restantes");
            loser.sendMessage("§c" + winner.getName() + " venceu o 1v1 com " + restingLife + " cora\u00e7oes e " + restingSoups + " sopas restantes");
            this.battle.remove(winner.getUniqueId());
                    ...
                    if (t != null) {
                        ...
                        if (menu.getName().equalsIgnoreCase("§c1v1 contra " + t.getName())) {
                            ...
                            if (currentItem.getType() == Material.WOOD_SWORD) {
                                ...
                                menu.setItem(currentSlot, new ItemBuilder().type(Material.STONE_SWORD).name("§6Espada de Pedra").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de sua espada!", "" }).build());
                            }
                            else if (currentItem.getType() == Material.STONE_SWORD) {
                                this.swordType.put(p.getUniqueId(), Material.IRON_SWORD);
                                menu.setItem(currentSlot, new ItemBuilder().type(Material.IRON_SWORD).name("§6Espada de Ferro").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de sua espada!", "" }).build());
                            }
                            else if (currentItem.getType() == Material.IRON_SWORD) {
                                this.swordType.put(p.getUniqueId(), Material.DIAMOND_SWORD);
                                menu.setItem(currentSlot, new ItemBuilder().type(Material.DIAMOND_SWORD).name("§6Espada de Diamente").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de sua espada!", "" }).build());
                            }
                            else if (currentItem.getType() == Material.DIAMOND_SWORD) {
                                this.swordType.put(p.getUniqueId(), Material.WOOD_SWORD);
                                menu.setItem(currentSlot, new ItemBuilder().type(Material.WOOD_SWORD).name("§6Espada de Madeira").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de sua espada!", "" }).build());
                            }
                            else if (currentItem.getType() == Material.LEATHER_CHESTPLATE) {
                                this.armourType.put(p.getUniqueId(), Material.IRON_CHESTPLATE);
                                menu.setItem(currentSlot, new ItemBuilder().type(Material.IRON_CHESTPLATE).name("§eArmadura de Ferro").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de sua armadura!", "" }).build());
                            }
                            else if (currentItem.getType() == Material.IRON_CHESTPLATE) {
                                this.armourType.put(p.getUniqueId(), Material.DIAMOND_CHESTPLATE);
                                menu.setItem(currentSlot, new ItemBuilder().type(Material.DIAMOND_CHESTPLATE).name("§eArmadura de Diamante").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de sua armadura!", "" }).build());
                            }
                            else if (currentItem.getType() == Material.DIAMOND_CHESTPLATE) {
                                this.armourType.put(p.getUniqueId(), Material.GOLD_HELMET);
                                menu.setItem(currentSlot, new ItemBuilder().type(Material.GOLD_HELMET).name("§eSem Armadura").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de sua armadura!", "" }).build());
                            }
                            else if (currentItem.getType() == Material.GOLD_HELMET) {
                                this.armourType.put(p.getUniqueId(), Material.LEATHER_CHESTPLATE);
                                menu.setItem(currentSlot, new ItemBuilder().type(Material.LEATHER_CHESTPLATE).name("§eArmadura de Couro").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de sua armadura!", "" }).build());
                            }
                            else if (currentItem.getType() == Material.RED_MUSHROOM) {
                                this.recraftType.put(p.getUniqueId(), Material.CACTUS);
                                menu.setItem(currentSlot, new ItemBuilder().type(Material.CACTUS).name("§bRecrafts de Cocoabean").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de seu recraft!", "" }).build());
                            }
                            else if (currentItem.getType() == Material.CACTUS) {
                                this.recraftType.put(p.getUniqueId(), Material.RED_MUSHROOM);
                                menu.setItem(currentSlot, new ItemBuilder().type(Material.RED_MUSHROOM).name("§bRecrafts de Cogumelo").lore(new String[] { "§3Clique aqui para mudar", "§3o tipo de seu recraft!", "" }).build());
                            }
                            else if (currentItem.getType() == Material.BROWN_MUSHROOM) {
                                this.recraftOption.put(p.getUniqueId(), false);
                                menu.setItem(currentSlot, new ItemBuilder().type(Material.MAGMA_CREAM).name("§cSem Recraft").lore(new String[] { "§3Clique aqui para", "§3ativar o recraft!", "" }).build());
                            }
                            else if (currentItem.getType() == Material.MAGMA_CREAM) {
                                this.recraftOption.put(p.getUniqueId(), true);
                                menu.setItem(currentSlot, new ItemBuilder().type(Material.BROWN_MUSHROOM).name("§aCom Recraft").lore(new String[] { "§3Clique aqui para", "§3desativar o recraft!", "" }).build());
                            }
                            else if (currentItem.getType() == Material.ENCHANTED_BOOK) {
                                this.sharpOption.put(p.getUniqueId(), false);
                                menu.setItem(currentSlot, new ItemBuilder().type(Material.BOOK).name("§3Sem Sharpness").lore(new String[] { "§3Clique aqui para", "§3colocar afia\u00e7ao na espada!", "" }).build());
                            }
                            else if (currentItem.getType() == Material.BOOK) {
                                this.sharpOption.put(p.getUniqueId(), true);
                                menu.setItem(currentSlot, new ItemBuilder().type(Material.ENCHANTED_BOOK).name("§3Com Sharpness").lore(new String[] { "§3Clique aqui para", "§3tirar a afia\u00e7ao da espada!", "" }).build());
                            }
                            else if (currentItem.getType() == Material.MUSHROOM_SOUP) {
                                this.fullSoupOption.put(p.getUniqueId(), false);
                                menu.setItem(currentSlot, new ItemBuilder().type(Material.BOWL).name("§21 Hotbar").lore(new String[] { "§3Clique aqui para", "§3usar full sopa", "" }).build());
                            }
                            else if (currentItem.getType() == Material.BOWL) {
                                this.fullSoupOption.put(p.getUniqueId(), true);
                                menu.setItem(currentSlot, new ItemBuilder().type(Material.MUSHROOM_SOUP).name("§2Full Sopa").lore(new String[] { "§3Clique aqui para", "§3usar 1 hotbar apenas", "" }).build());
                            }
                                ...
                                new BukkitRunnable() {
                                    public void run() {
                                        if (BattleMinigame.this.customChallenge.containsKey(t.getUniqueId()) && BattleMinigame.this.customChallenge.get(t.getUniqueId()) == p.getUniqueId()) {
    ...
    private void prepareCustomBattle(final Player customer, final Player challenged) {
        ...
        if (armour == Material.LEATHER_CHESTPLATE) {
            ...
            final Material gold_HELMET = Material.GOLD_HELMET;
        }
    ...
    private void prepareNormalBattle(final Player p) {
        ...
        if (yPvP.getPlugin().getPvpType() == yPvP.PvPType.FULLIRON) {
            ...
            builder = new ItemBuilder().type(Material.DIAMOND_SWORD).enchantment(Enchantment.DAMAGE_ALL, Integer.valueOf(1));
            p.getInventory().setItem(0, builder.build());
            builder = null;
        }
        else {
            ItemBuilder builder = new ItemBuilder().type(Material.WOOD_SWORD).enchantment(Enchantment.DAMAGE_ALL, Integer.valueOf(1));
            p.getInventory().setItem(0, builder.build());
            builder = null;
        }
        p.updateInventory();
