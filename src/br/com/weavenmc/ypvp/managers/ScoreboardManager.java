...
public class ScoreboardManager extends Management implements Listener
{
    ...
    private String titleSpawn;
    private String title1v1;
    ...
    @Override
    public void enable() {
        this.scrollerSpawn = new StringScroller("NestyPvP - Spawn - " + this.getPlugin().getPvpType().name() + " -", 12, 1);
        this.titleSpawn = this.scrollerSpawn.next();
        this.scroller1v1 = new StringScroller("NestyPvP - 1v1 - " + this.getPlugin().getPvpType().name() + " -", 12, 1);
        this.title1v1 = this.scroller1v1.next();
        this.scrollerFps = new StringScroller("NestyPvP - Fps - " + this.getPlugin().getPvpType().name() + " -", 12, 1);
        this.titleFps = this.scrollerFps.next();
        this.scrollerChallenge = new StringScroller("NestyPvP - Lava Challenge - " + this.getPlugin().getPvpType().name() + " -", 12, 1);
        this.titleChallenge = this.scrollerChallenge.next();
        this.scrollerVoid = new StringScroller("NestyPvP - Void Challenge - " + this.getPlugin().getPvpType().name() + " -", 12, 1);
        this.titleVoid = this.scrollerVoid.next();
    ...
    public void updateTab(final Player p, final int onlineCount) {
        ...
        TabListAPI.setHeaderAndFooter(p, "§6§lNESTY§f§lMC §e§lKITPVP\n§eMoedas: §f" + bP.getMoney() + " §9- §eTickets: §f" + bP.getTickets() + " §9- §ePing: §f" + ping + "\n" + "§eTemos §f" + onlineCount + "§e jogadores online!", "§bNick: §f" + bP.getName() + " §9- §bLiga: §f" + bP.getLeague().name() + " §9- §bXP: §f" + bP.getXp() + "\n§bMais informa\u00e7\u00f5es em: §fwww.nestymc.com.br");
    }
    
    @Override
    public void disable() {
        this.scrollerSpawn = null;
        this.titleSpawn = null;
    }
}
